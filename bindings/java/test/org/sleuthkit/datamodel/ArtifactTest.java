...
public class ArtifactTest {
	...
	public void artifactTests() throws TskCoreException, Blackboard.BlackboardException, OsAccountManager.NotUserSIDException {
		...
		DataArtifact dataArtifact1 = abcTextFile.newDataArtifact(BlackboardArtifact.Type.TSK_GPS_SEARCH, Collections.emptyList(), osAccount1.getId());
		...
		DataArtifact dataArtifact2 = abcTextFile.newDataArtifact(BlackboardArtifact.Type.TSK_CLIPBOARD_CONTENT, Collections.emptyList(), osAccount2.getId());
		...
		// and two more 
		DataArtifact dataArtifact3 = abcTextFile.newDataArtifact(BlackboardArtifact.Type.TSK_GPS_AREA, Collections.emptyList(), osAccount2.getId());
		DataArtifact dataArtifact4 = abcTextFile.newDataArtifact(BlackboardArtifact.Type.TSK_GPS_AREA, Collections.emptyList(), osAccount2.getId());

		
		// TEST: get all TSK_GPS_SEARCH data artifacts in the data source
		List<DataArtifact> gpsArtifacts = caseDB.getBlackboard().getDataArtifacts(BlackboardArtifact.Type.TSK_GPS_SEARCH.getTypeID(), image.getId());
		...
		List<DataArtifact> gpsAreaArtifacts = caseDB.getBlackboard().getDataArtifacts(BlackboardArtifact.Type.TSK_GPS_AREA.getTypeID(), image.getId());
